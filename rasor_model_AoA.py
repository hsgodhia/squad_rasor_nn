...
class SquadModel(nn.Module):
    ...
    def forward(self, config, p, p_mask, p_lens, q, q_mask, q_lens):
        ...
        q_align_weights = self.sequence_softmax(q_align_mask_scores)  #softmax across the q_len index=2, this is what we do in eqn(7); (batch_size, max_p_len, max_q_len)

        q_align_weights_row = self.sequence_softmax2(q_align_mask_scores) #softmax across the p_len index=1, this is what we do for AoA model; (batch_size, max_p_len, max_q_len)
    ...
    #Used for computing row-wise softmax for q_aligned
